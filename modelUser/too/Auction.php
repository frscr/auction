...
class Auction
{
	...
	function select_lot($lot_id)
	{
		...
		$sql = "SELECT tbl_lots.title AS name,
				tbl_bets.start_price AS start_price,
				tbl_bets.max_price AS buyout,
				tbl_bets.real_price AS real_price,
				tbl_bets.step AS step,
				tbl_bets.id AS id_bet  
				FROM tbl_lots
				INNER JOIN tbl_bets ON tbl_lots.id_bet = tbl_bets.id WHERE tbl_lots.id = '$lot_id'";
		$connect = mysqli_connect($this->host, $this->user, $this->password, $this->dbname) 
	...
	{
		$list_lots = "";
		$sql = "SELECT tbl_lots.description AS lot_descr, 
		        tbl_lots.id AS lot_id, 
		        tbl_lots.title AS lot_titl, 
		        tbl_bets.start_price AS bet_start, 
		        tbl_bets.step AS bet_step, 
		        tbl_bets.real_price AS bet_price 
		        FROM tbl_lots
	...
	{
		$list_lots = "";
		$sql = "SELECT tbl_lots.description AS descr,
				tbl_lots.title AS title, tbl_lots.id AS lot_id,
				tbl_bets.max_price AS max 
				FROM tbl_lots
...
//Функция отвечает за выбор ставки
	function bet($bet_id, $step)
	{
		if(!preg_match("/^\d+$/", $bet_id) || !preg_match("/^\d+$/", $step)){
			exit("Error");
		}
		$sql_select = "SELECT * FROM tbl_bets WHERE id='$bet_id'";
     	$connect = mysqli_connect($this->host, $this->user, $this->password, $this->dbname)
     		or die("Ошибка". mysqli_error($connect));
 		if($result = mysqli_query($connect, $sql_select)) {
 			$data = mysqli_fetch_assoc($result);
 		}
 		$result_step = $step + $data['real_price'];
 		$sql_update = "UPDATE tbl_bets SET real_price = '$result_step', id_last_user ='$this->user_id' WHERE id = '$bet_id'";
